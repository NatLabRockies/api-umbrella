...
local AgentLoopTarget = model_ext.new_class("agent_loop_targets", {
  as_json = function(self)
    return {
      ...
      current_grade = json_null_default(tonumber(self.current_grade)),
      current_grade_label = json_null_default(self.current_grade_label),
      reputation_score = json_null_default(tonumber(self.reputation_score)),
      asset_value = json_null_default(tonumber(self.asset_value)),
      last_event_at = json_null_default(time.postgres_to_iso8601(self.last_event_at)),
      iteration_count = json_null_default(tonumber(self.iteration_count)),
...
AgentLoopTarget.standings = function()
  ...
  for _, row in ipairs(rows) do
    table.insert(standings, {
      ...
      average_grade = json_null_default(tonumber(row.average_grade)),
      reputation_score = json_null_default(tonumber(row.reputation_score)),
      asset_value = json_null_default(tonumber(row.asset_value)),
